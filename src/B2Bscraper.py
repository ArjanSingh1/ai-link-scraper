...
class B2BVaultAgent:
    ...
    def __init__(self, output_dir: str = "scraped_data", tabs_to_search: List[str] = None, max_workers: int = 5):
        ...
        votes_file = os.path.join(output_dir, "votes.json")
        self.voting_system = VotingSystem(votes_file)
    ...
    def generate_website(self, processed_articles: List[Dict], pdf_path: str = None, preview: bool = False):
        """Generate a static website to display all analyzed articles with voting functionality."""
        self.logger.info("Generating static website with voting system")
        if preview:
            print("🌐 Generating static website with upvote/downvote functionality...")
        
        # Create website directory
        website_dir = os.path.join(self.output_dir, "website")
        os.makedirs(website_dir, exist_ok=True)
        
        # Generate article cards HTML with voting
        articles_html = ""
        for i, article in enumerate(processed_articles):
            ...
            # Get current vote counts for this article
            vote_data = self.voting_system.get_votes(article['url'])
            ...
            <div class="article-card" id="article-{i}">
                <div class="article-header">
                    ...
                    <div class="voting-section">
                        <button class="vote-btn upvote-btn" onclick="vote('{article['url']}', 'upvote', {i})" id="upvote-{i}">
                            👍 <span id="upvote-count-{i}">{vote_data['upvotes']}</span>
                        </button>
                        <span class="vote-score" id="score-{i}">{vote_data['score']:+d}</span>
                        <button class="vote-btn downvote-btn" onclick="vote('{article['url']}', 'downvote', {i})" id="downvote-{i}">
                            👎 <span id="downvote-count-{i}">{vote_data['downvotes']}</span>
                        </button>
                    </div>
                </div>
        ...
        <body>
            ...
            <div class="container">
                <div class="header">
                    ...
                    <p>AI-powered B2B article summaries with community voting</p>
                    ...
                    <div class="stats">
                        ...
                        <div class="stat-card">
                            <span class="stat-number">{sum(self.voting_system.get_votes(a['url'])['total_votes'] for a in processed_articles)}</span>
                ...
                <div class="footer">
                    <p>Generated on {time.strftime('%Y-%m-%d %H:%M:%S')} | Powered by Perplexity AI & Community Voting</p>
            ...
            <script>
                // Generate a simple user ID for voting (stored in localStorage)
                let userId = localStorage.getItem('b2b_user_id');
                if (!userId) {{
                    userId = 'user_' + Math.random().toString(36).substr(2, 9);
                    localStorage.setItem('b2b_user_id', userId);
                }}
                ...
                async function vote(articleUrl, voteType, articleIndex) {{
                    try {{
                        // Simulate API call (in a real implementation, this would call a backend)
                        const response = await simulateVote(articleUrl, voteType, userId);
                        
                        if (response.success) {{
                            // Update the UI
                            document.getElementById(`upvote-count-${{articleIndex}}`).textContent = response.upvotes;
                            document.getElementById(`downvote-count-${{articleIndex}}`).textContent = response.downvotes;
                            document.getElementById(`score-${{articleIndex}}`).textContent = response.score >= 0 ? `+${{response.score}}` : response.score;
                            
                            // Show notification
                            const actionText = response.action === 'removed' ? 'removed' : response.action === 'changed' ? 'changed' : 'added';
                            showNotification(`Vote ${{actionText}} successfully! Score: ${{response.score}}`);
                            
                            // Add visual feedback
                            const btn = document.getElementById(`${{voteType}}-${{articleIndex}}`);
                ...
                function simulateVote(articleUrl, voteType, userId) {{
                    // In a real implementation, this would make an actual API call
                    // For now, we'll simulate the vote by storing in localStorage
                    return new Promise((resolve) => {{
                        let votes = JSON.parse(localStorage.getItem('article_votes') || '{{}}');
                        
                        if (!votes[articleUrl]) {{
                            votes[articleUrl] = {{ upvotes: 0, downvotes: 0, voters: {{}} }};
                        }}
                        
                        const article = votes[articleUrl];
                        let action = 'added';
                        
                        if (userId in article.voters) {{
                            const previousVote = article.voters[userId];
                            
                            if (previousVote === voteType) {{
                                // Remove vote
                                delete article.voters[userId];
                                article[voteType + 's']--;
                                action = 'removed';
                            }} else {{
                                // Change vote
                                article[previousVote + 's']--;
                                article[voteType + 's']++;
                                article.voters[userId] = voteType;
                                action = 'changed';
                            }}
                        }} else {{
                            // New vote
                ...
                // Load existing votes from localStorage on page load
                window.addEventListener('load', () => {{
                    const votes = JSON.parse(localStorage.getItem('article_votes') || '{{}}');
                    // In a real implementation, you would load votes from the server
        ...
        try:
            ...
            # Create a simple voting API server script
            api_server_script = f"""#!/usr/bin/env python3
import json
import http.server
import socketserver
...
# Add the parent directory to the path so we can import voting_system
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from voting_system import VotingSystem

class VotingAPIHandler(http.server.SimpleHTTPRequestHandler):
    def __init__(self, *args, **kwargs):
        self.voting_system = VotingSystem('{os.path.join(self.output_dir, "votes.json")}')
    ...
    def handle_vote(self):
        try:
            ...
            result = self.voting_system.vote(article_url, vote_type, user_id)
    ...
    def handle_get_votes(self):
        try:
            query = urlparse(self.path).query
            params = parse_qs(query)
            
            if 'article_url' in params:
                article_url = params['article_url'][0]
                result = self.voting_system.get_votes(article_url)
            else:
                result = self.voting_system.get_all_votes()
...
if __name__ == "__main__":
    PORT = 8001
    with socketserver.TCPServer(("", PORT), VotingAPIHandler) as httpd:
        print(f"🗳️  Voting API server running at http://localhost:{{PORT}}")
        print("📊 API endpoints:")
        print("  POST /api/vote - Cast a vote")
        print("  GET /api/votes - Get all votes")
        print("  GET /api/votes?article_url=URL - Get votes for specific article")
        print("⏹️  Press Ctrl+C to stop")
        httpd.serve_forever()
"""
            
            api_path = os.path.join(website_dir, "voting_api.py")
            with open(api_path, "w", encoding="utf-8") as f:
                f.write(api_server_script)
            
            # Generate the main launcher script
...
def start_voting_api():
    try:
        subprocess.run(['python3', 'voting_api.py'], cwd=r"{website_dir}")
    except:
        pass
...
def start_website():
    ...
    try:
        with socketserver.TCPServer(("", PORT), Handler) as httpd:
            print(f"🌐 B2B Vault Analysis Website with Voting")
            print(f"🚀 Website: http://localhost:{{PORT}}")
            print("🗳️  Voting system enabled with local storage")
...
if __name__ == "__main__":
    # Start the voting API in a separate thread (optional, for real-time voting)
    # api_thread = threading.Thread(target=start_voting_api, daemon=True)
    # api_thread.start()
...
"""
            ...
            if preview:
                print(f"   ✅ Website with voting generated: {website_path}")
                print(f"   🗳️  Voting API: {api_path}")
                print(f"   🚀 Server script: {server_path}")
                print(f"   💡 To view the website, run:")
                print(f"       python3 {server_path}")
                print(f"   📊 Features:")
                print(f"       - Upvote/downvote buttons on each article")
                print(f"       - Real-time vote counting")
                print(f"       - Community-driven article ranking")
                print(f"       - Local storage persistence")
